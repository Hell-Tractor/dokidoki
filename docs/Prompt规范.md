...
+ T-07 低信息输入处理（对话回复常驻；由模型自行判断是否适用）
+ [场景附加，见下]
```
...
对话回复 **常驻** 注入；不由服务端做文本匹配。由模型自行判断本轮是否低信息。
...
- `char_busy`（`paused_char_busy` 忙完后重启）→ **T-15_CHAR_BUSY**
...
- `user_busy`（`paused_user_busy` 按曲线重启）→ **T-15_USER_BUSY**

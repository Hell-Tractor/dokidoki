...
| `api/mod.rs` | Axum Router 聚合 |
| `api/middleware.rs` | Bearer Token 鉴权（查 `user_sessions` → `user_id`） |
| `api/rest/` | auth、users、conversations、messages、settings、devices、health |
| `auth/mod.rs` | 密码哈希（argon2）、Token 生成、`user_sessions` 读写 |
| `api/ws/` | WebSocket 连接管理、subscribe、事件下发 |
| `db/mod.rs` | sqlx 查询封装；models |
| `db/models.rs` | 表行结构体 |
| `db/queries/` | 按领域拆分 SQL |
| `persona/mod.rs` | 读取 `persona_json`、渲染 Prompt 静态层 |
| `persona/prompt.rs` | 调用 Prompt 规范模板拼接 |
| `schedule/mod.rs` | 解析 schedule、计算 CurrentState |
| `schedule/resolver.rs` | 周模板 + 随机事件 |
| `chat/mod.rs` | 对话入口：收消息、触发 burst |
...
| `memory/mod.rs` | STORE/FORGET 执行、过期清理 |
| `proactive/mod.rs` | 主动消息触发判断与生成 |
| `proactive/triggers.rs` | 六类触发器实现 |
| `llm/mod.rs` | OpenAI 兼容 HTTP 客户端 |
| `llm/vision.rs` | 图片多模态请求 |
| `push/mod.rs` | FCM 注册与发送 |
| `scheduler/mod.rs` | 每分钟 tick；每日记忆清理 |
...
| 分层 | 目录 | 职责 | 禁止 |
|------|------|------|------|
| **传输层** | `api/` | HTTP/WS 路由、鉴权、请求/响应序列化、调用领域服务 | 不写 burst 合并、Prompt 组装、状态机等业务逻辑 |
| **领域服务** | `chat/` `proactive/` `schedule/` `persona/` `memory/` | 业务编排、领域规则、内存状态（burst、reply 队列） | 不直接拼 HTTP 响应；不散落 SQL 字符串 |
| **仓储** | `db/` | sqlx 查询、`models`、事务边界 | 不调用 `chat`/`proactive` 等上层模块 |
...
| DTO/VO | `api` 内 request/response struct | 仅在与 DB model 形态不同时定义 |
...
- Entity / DTO / VO 三层手动转换（除非 API 与表结构显著不同）
- 为简单 CRUD 再包 `BaseDao`
...
- 种子数据：`migrations/seeds/dev_characters.sql`（测试角色，非 migration）
...
|------|------|
| 形态 | Tab 或分段切换「注册」「登录」 |
| 注册 | `username`、`password`（≥8 位）、`display_name`（可选）、`birthday`（可选）→ `POST /auth/register` |
...
当会话 turn 数 > 阈值（默认 30）时触发摘要 LLM 调用。

### 11.2 流程

1. 取最早 (总数 - 保留数) 轮消息 → LLM 生成摘要
2. 追加/合并到 `conversations.summary`
3. 保留最近 10 轮原文进入历史层
4. 摘要与记忆分工：摘要=上下文压缩；记忆=可引用事实

...
| 字段 | 值 |
|------|-----|
| `conversations.status` | `active`, `winding_down`, `paused`, `paused_char_busy`, `paused_user_busy` |
| `conversations.winding_reason` | `NULL`, `normal`, `char_busy`, `user_busy` |
...
| `proactive_logs.trigger_type` | `schedule_change`, `silence_wake`, `re_engage`, `daily_greeting`, `mood_followup`, `special_date`, `pre_sleep` |
...
| `reply_delay_factor` | float \| float[2] | 否 | 回复延迟性格系数 `[min,max]`，缺省 `1.0` |
| `conversation_style` | string | 否 | 对话倾向自然语言描述，注入 T-10 |
| `conversation_behavior.skip_reply_tendency` | enum | 否 | `low` / `medium` / `high` |
| `conversation_behavior.end_topic_freely` | bool | 否 | 是否主动结束话题 |
| `conversation_behavior.re_engage_after_minutes` | int | 否 | **废弃（兼容可读）**；话题重启不再用固定等待 |
| `conversation_behavior.pause_on_farewell` | bool | 否 | winding_down 下用户告别后是否进入终态，默认 `true` |
| `proactive.silence_after_hours` | float | 否 | 沉默唤醒阈值（小时），仅 `status=paused` |
| `proactive.probability_factor` | float | 否 | 主动触发概率系数，默认 `1.0` |
| `proactive.schedule_change_probability` | float | 否 | 日程切换本段触发倾向，默认 `0.35`；× availability base × `probability_factor`，每段一次 |
| `proactive.user_busy_reengage.min_delay_minutes` | int | 否 | 用户忙暂停后多久内 P=0，默认 30 |
| `proactive.user_busy_reengage.ramp_minutes` | int | 否 | 线性爬升时长，默认 90 |
| `proactive.user_busy_reengage.peak_probability` | float | 否 | 爬升上限，默认 0.6 |
...
| `weekly_template[].kind` | enum | 否 | `wake` / `sleep` / `custom`（缺省 `custom`）；`wake` 用于每日问候；`sleep` 内禁一切 proactive |
| `random_events.probability` | float | 否 | 日触发概率，默认 0.15 |
| `random_events.pool` | string[] | 否 | 随机事件文案池 |

**约定（后续加强校验）**：同一日模板时段不重叠；每段必有 `end`。日初对 start/end 做扰动（延后）。`pre_sleep`：切入 sleep 前晚安（M-07 本期做）。
...
| 风格层 | T-06, T-10, T-07 | 繁忙程度、性格倾向、低信息处理指引（T-07 对话常驻，模型自判） | 每轮对话回复 |
| 场景层 | T-08～T-09, T-11 | 图片、winding_down、情绪低落 | 按条件 |
| 历史层 | — | 近期原文消息 | 每轮 |
...
对话回复 System 常驻注入 T-07：由 LLM 自行判断本轮是否低信息（语气词、emoji、极短无实义等），走陪伴模式；有实质内容则按正常聊天忽略该段。**服务端不做关键词/长度匹配门控。**
...
```
reply_wait = sample(availability) × reply_delay_factor × jitter
jitter = uniform(config.jitter_min, config.jitter_max)
reply_delay_factor = uniform(persona.reply_delay_factor[0], persona.reply_delay_factor[1])  # 缺省 1.0
```

### 8.2 reply_delay_factor

来自角色 `persona_json.reply_delay_factor`（单个数字或 `[min, max]`），不再通过性格枚举映射。
...
| 状态 | 含义 | proactive |
|------|------|-----------|
| `active` | 正常对话 | 允许（受 sleep / 闸门约束） |
| `winding_down` | 话题收束中；双方还可再聊几句告别 | **禁止** |
| `paused` | **话题正常结束**，不必为「同一话题」重启；下次聊天开新话题 | 仅 `silence_wake` |
| `paused_char_busy` | **角色去忙**导致话题异常中断，需要重启 | `re_engage`（忙完） |
| `paused_user_busy` | **用户去忙**导致话题异常中断，需要重启 | `re_engage`（人设曲线） |

辅助字段：

| 字段 | 说明 |
|------|------|
| `paused_at` | 进入任一「暂停类」终态（`paused` / `paused_*_busy`）的时间 |
| `winding_reason` | 可空；进入 `winding_down` 时写入：`normal` \| `char_busy` \| `user_busy`；落地终态后清空 |

```puml
@startuml
[*] --> active : 创建会话

active --> active : LLM [NO_REPLY] / [REPLY]
active --> winding_down : [END_TOPIC]\n(reason=normal|char_busy)
active --> winding_down : 检测 [USER_BUSY]\n(reason=user_busy)

winding_down --> active : 用户 substantive 新话题
winding_down --> paused : 告别/超时\nreason=normal
winding_down --> paused_char_busy : 告别/超时\nreason=char_busy
winding_down --> paused_user_busy : 告别/超时\nreason=user_busy

paused --> active : 用户 substantive / silence_wake
paused_char_busy --> active : 用户 substantive / re_engage
paused_user_busy --> active : 用户 substantive / re_engage

note right of winding_down
  不可 proactive
  超时默认 5min（全局 config）
end note
@enduml
...
### 9.2 进入 winding_down 的原因

| `winding_reason` | 条件 | Prompt 约束 |
|------------------|------|-------------|
| `char_busy` | LLM `[END_TOPIC]` **且** 当时角色 `availability=low` | 文案须明确说明自己要去忙（上课/做事等）；**允许多聊几句**再告别 |
| `user_busy` | 主回复链路中模型输出 **`[USER_BUSY]` tag**（与动作头并存或同轮标记；有上下文，比规则更准） | 之后可再聊几句收尾（如用户「等下再聊」→ 角色「你先去」→ 用户「拜拜」→ `[NO_REPLY]`） |
| `normal` | `[END_TOPIC]` 且非 `char_busy` | 正常收束，不要求「去忙」话术 |

> 不新增 `winding_down_*` 状态；用 `winding_reason` 区分即可。

### 9.3 转换表

| 当前 | 事件 | 下一 | 动作 |
|------|------|------|------|
| active | `[NO_REPLY]` / `[REPLY]` | active | 常规投递 |
| active | `[END_TOPIC]` + availability=low | winding_down | `winding_reason=char_busy`；多气泡 |
| active | `[END_TOPIC]` 其它 | winding_down | `winding_reason=normal` |
| active | 输出含 `[USER_BUSY]` | winding_down | `winding_reason=user_busy`；仍可正常回复 |
| winding_down | 用户 substantive | active | 清空 `winding_reason` |
| winding_down | 告别短句（+ `pause_on_farewell`）或 **超时** | 见 reason | 写 `paused_at`；清 `winding_reason` |
| paused | 用户 substantive / `silence_wake` | active | — |
| paused_char_busy / paused_user_busy | 用户 substantive / `re_engage` | active | 清 `paused_at` |

**超时**：`config.toml`（如 `winding_down_timeout_secs = 300`，默认 5 分钟）。超时落地规则与告别落地相同（按 `winding_reason`）。

### 9.4 告别类短句识别

规则预筛：`好的`、`拜拜`、`去吧`、`嗯嗯` 等；`winding_down` 下按 `conversation_behavior.pause_on_farewell` 决定是否进入对应终态。

### 9.5 sleep 与 `pre_sleep`

- **当前槽 `kind=sleep`**：禁止一切 proactive（含已在 sleep 内的 `re_engage`）。
- 日程时段**不重叠**；槽位可首尾相接（如 22:30 下课接睡觉）。
- **`pre_sleep`（本期）**：切入 sleep **之前**发晚安；`paused_char_busy` 时按性格决定是否关心用户忙完没。覆盖首尾相接、不宜立刻开聊的边界；睡醒后再 `daily_greeting` / `re_engage`。
...
同 tick 每会话只取优先级最高的一条：`pre_sleep` → `daily_greeting` → `re_engage` → `silence_wake` → `mood_followup` → `schedule_change`。

| 类型 | 条件 |
|------|------|
| 睡前 `pre_sleep` | 即将切入 `kind=sleep`（上一槽结束 / 进入 sleep 前的短窗）；每角色每日至多一次；`paused_char_busy` 时可叠「忙完没」关心 |
| 每日问候 | 当前槽 `kind=wake` 的问候窗内；每角色每日一次；可叠生日等 special |
| 话题重启 `re_engage` | **`paused_char_busy`**：`now ≥ character_states.activity_ends_at`（非 sleep）后走概率闸门；**`paused_user_busy`**：按 persona `user_busy_reengage` 对 \(t=now-paused_at\) 算 \(P(t)\) 后抽样。**不**从 `paused` 触发 |
| 沉默唤醒 | **`status=paused`** 且距用户末条消息 ≥ `proactive.silence_after_hours`（缺省 8h）且 availability ≥ medium |
| 日程切换 | `status=active` 且刚进入 `kind=custom`；`schedule_change_probability` × 全局闸门（每段一次确定性抽样） |
| 情绪跟进 | 上次对话标记负面情绪且超过冷却 |
| 特殊日期 | 并入 `daily_greeting`（用户生日等）；公共节日方案暂缓 |

### 13.3 全局过滤（任一不满足则跳过）

- 当前日程槽 **`kind=sleep`**（禁止一切 proactive）
- 当前时间在勿扰时段内
- 当日主动消息数 ≥ `users.max_proactive_per_day`
- `status=winding_down`（收束中不主动）
- availability = low 时降低/跳过概率（`re_engage` 在 char_busy 忙完后通常已非 low）
- `proactive.probability_factor` 调节触发概率

### 13.3.1 `user_busy_reengage` 概率曲线（persona）

仅用于 `paused_user_busy`：

```yaml
user_busy_reengage:
  min_delay_minutes: 30   # t 小于此 → P=0
  ramp_minutes: 90        # 之后线性爬升到 peak
  peak_probability: 0.6   # 上限；再乘全局/availability 闸门
```

黏人角色：短 `min_delay`、陡 `ramp`、高 `peak`；淡定则相反。
计算：`t = now - paused_at`（分钟）。`t < min_delay` → P=0；随后在 `ramp_minutes` 内线性升至 `peak_probability`；之后维持 peak。抽样时再乘全局/availability 闸门。

### 13.3.2 `pre_sleep`

切入 `kind=sleep` **之前**发晚安（不在 sleep 槽内发 proactive）。`paused_char_busy` 时按性格决定是否问用户忙完。用于忙段与 sleep 首尾相接；计入日上限与 `proactive_logs.trigger_type=pre_sleep`。
...
在 schedule 起床活动段内，在 30–60 分钟窗内随机触发；与用户是否打开 App **无关**。语气遵循 `conversation_style`。
...
| v1.4 | 2026-07-14 | 会话状态扩展（paused vs paused_*_busy）、winding_reason、re_engage/silence_wake 分工、sleep 禁 proactive、pre_sleep 本期、日初扰动延后 |

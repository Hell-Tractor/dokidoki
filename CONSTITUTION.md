...
后端统一使用 `tracing`。业务路径上的**可控跳过**与**软失败**必须打日志，不能只靠返回 `Ok` / 空结果 silently 通过。

### 11.1 级别

| 级别 | 用途 |
|------|------|
| `error` | 意外故障、基础设施异常、不可恢复错误（序列化失败、不可用依赖等） |
| `warn` | **可控软失败**或坏数据：LLM 失败、空气泡、非法 JSON、配置缺失；本轮跳过但不崩进程 |
| `info` | 重要业务结果：破冰/主动消息投递成功、投递取消、摘要落库、记忆写入/删除、`END_TOPIC` / `NO_REPLY`、状态切 paused 等 |
| `debug` | 闸门未过、条件不满足但值得排障：勿扰、日上限、概率未过、起床窗外、日程 refresh 结果 |
| `trace` | 高频且预期的空转：无触发器命中、非 `wake` 时段、今日已问候 |

### 11.2 内容要求

- 带上足以定位的实体 ID：`conversation_id` / `user_id` / `character_id` / `turn_id`（视场景）
- 说明**为什么跳过或失败**（原因短句），不要只写 `failed`
- **禁止**记录密钥、Token、完整密码、无必要的 Prompt / 用户隐私正文（必要时只记长度或 hash 摘要）
- 对客户端返回的 `AppError` **不能替代**服务端日志：`Ok` 路径上的受控跳过仍要 log

### 11.3 禁止与节制

- 不要在超热路径用 `info` 刷屏（如每个 schedule tick 对每个角色打 info）
- 解析失败（`persona_json` / `schedule_json`）须 `warn` 并带 `character_id`
- 后台任务单实体失败应隔离：`warn` 后继续其它实体，不中断整轮 tick

### 11.4 与测试

- 单元测试不依赖日志断言；集成调试时可临时提高 `RUST_LOG`
